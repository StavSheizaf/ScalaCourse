...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    b.legalNeighbors.map(move => (move._1, move._2 :: history)).to(LazyList)
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] =
    if initial.isEmpty then LazyList.empty else
      def moves = initial.flatMap(path => newNeighborsOnly(neighborsWithHistory(path._1, path._2), explored))
      initial #::: from(moves, explored ++ moves.map(_._1))
