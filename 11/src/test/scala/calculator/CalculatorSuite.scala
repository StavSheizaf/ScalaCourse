...
class CalculatorSuite extends munit.FunSuite {
  ...
  test("Expressions corresponding to every variable should be correctly evaluated") {
    ...
    val output = computeValues(input)
    var dres1 = output("d")() == 1
    assert(dres1, " - Value of `d` is incorrect!")
    var cres1 = output("c")() == 6
    assertEquals(output("c")(), 6.0)
    assert(cres1, " - Value of `c` is incorrect!")
    var bres1 = output("b")() == 6
    assert(bres1, " - Value of `b` is incorrect!")
