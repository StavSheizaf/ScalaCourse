...
trait DecoderInstances:
  ...
  given Decoder[Int] =
    Decoder.fromPartialFunction { case Json.Num(x) if x.isValidInt => x.intValue  }

  /** A decoder for `String` values */
  given Decoder[String] =
    Decoder.fromPartialFunction { case Json.Str(x) => x }


  /** A decoder for `Boolean` values */
  given Decoder[Boolean] =
    Decoder.fromPartialFunction { case Json.Bool(x) => x }
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction {
      case Json.Arr(x) =>
        val list = x.map(item => decoder.decode(item))
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction { case Json.Obj(x) if x.contains(name) => decoder.decode(x(name))}
...
trait PersonCodecs:
  ...
  given Decoder[Person] =
    Decoder.field[String]("name")
      ...
      .transform[Person]((a,b) => Person(a, b))
...
trait ContactsCodecs:
  ...
  given Encoder[Contacts] =
    ObjectEncoder.field[List[Person]]("people").transform[Contacts]{ case Contacts(people) => people }
